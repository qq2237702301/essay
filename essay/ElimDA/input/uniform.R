...
q1<-ggplot(x1, aes(x=nodetype, y=aver, group=methodtype, shape=methodtype, color=methodtype)) +
  ...
  scale_linetype_manual(name=element_blank(), 
                        breaks=c("a", "b", "c", "d", "e", "f", "g"),
                        values=c(a=3, b=2, c=4, d=5, e=1, f=2,g=1), 
                        labels=c("DASH", "CCEH", "Level", "Clevel", "Plush","Viper","ElimDA")) +
  ...
  # geom_line(size=1.2, aes(linetype=methodtype,group=methodtype)) +
...
q2<-ggplot(x2, aes(x=nodetype, y=aver, group=methodtype, shape=methodtype, color=methodtype)) +
  # theme_bw() +
  scale_color_manual(name=element_blank(), 
                     breaks=c("a","b", "c", "d", "e","f","g"), 
                     values=c(a="#63b235", b="#ed7d31", c="#5A5AAD", d="#2e75b6", e="#800080",f="#adab00",g="#c00000"), 
  ...
  # scale_x_discrete(breaks=c("a", "b", "c", "d"), 
  #                  labels=c("1", "2", "4","8")) +
  scale_y_continuous(expand=c(0,0), limits=c(0, 36), breaks=c(0, 8, 16, 24, 32))+
  # geom_line(size=1.2, aes(linetype=methodtype,group=methodtype)) +
  geom_point(size=5, stroke=3, aes(shape=methodtype)) +
  geom_errorbar(aes(ymin=aver, ymax=aver), width=.1, position=position_dodge(0)) + 
  ylab("Throughput (Mops/s)") +
...
q3<-ggplot(x3, aes(x=nodetype, y=aver, group=methodtype, shape=methodtype, color=methodtype)) +
  scale_color_manual(name=element_blank(), 
                     breaks=c("a","b", "c", "d", "e","f","g"), 
                     values=c(a="#63b235", b="#ed7d31", c="#5A5AAD", d="#2e75b6", e="#800080",f="#adab00",g="#c00000"), 
                     labels=c("DASH", "CCEH", "Level", "Clevel", "Plush", "Viper" ,"ElimDA")) + 
...
q4<-ggplot(x4, aes(x=nodetype, y=aver, group=methodtype, shape=methodtype, color=methodtype)) +
  scale_color_manual(name=element_blank(), 
                     breaks=c("a","b", "c", "d", "e","f","g"), 
                     values=c(a="#63b235", b="#ed7d31", c="#5A5AAD", d="#2e75b6", e="#800080",f="#adab00",g="#c00000"), 
                     labels=c("DASH", "CCEH", "Level", "Clevel", "Plush", "Viper" ,"ElimDA")) + 
  ...
  # scale_x_discrete(breaks=c("a", "b", "c", "d"), 
  #                  labels=c("1", "2", "4","8")) +
  scale_y_continuous(expand=c(0,0), limits=c(0, 22), breaks=c(0, 4, 8, 12, 16, 20))+
  # geom_line(size=1.2, aes(linetype=methodtype,group=methodtype)) +
  geom_point(size=5, stroke=3, aes(shape=methodtype)) +
  geom_errorbar(aes(ymin=aver, ymax=aver), width=.1, position=position_dodge(0)) + 
  ylab("Throughput (Mops/s)") +
...
q5<-grid_arrange_shared_legend(q1, q2, q3, q4,ncol = 4, nrow = 1,position='top')
	ggsave("D:/essay/ElimDA/output/uniform.pdf", q5, width=10, height=5, device=cairo_pdf)