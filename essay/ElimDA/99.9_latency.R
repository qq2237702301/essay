...
x1<-read.table("C:/Users/yzx/Desktop/ElimDA_Paper/ICDE/evaluation/Latency/99.9_latency.txt", header=TRUE,  sep="\t")
library(ggplot2)
library(grid)
library(extrafont)
...
q1 = ggplot(x1, aes(x=codetype, y=aver, fill=methodtype)) + 
  ...
  geom_bar_pattern(stat = "identity", 
                   ...
                   pattern_angle = rep(c(rep(45, 1), rep(0,1), rep(135,1), rep(0, 1), rep(135, 1),rep(135,1),rep(135,1)),4),   #跟x有关系
                   #pattern_angle=45,
                   pattern_density = .1,
                   pattern_spacing = .03,
                   pattern_fill = 'black',
  ...
  geom_text(aes(label=sprintf("%.1f", round(aver,digit=1)) ), position=position_dodge(.85),hjust=0.46, vjust=-0.5, size=5.5, angle=0) +
...
q2= gg.gap(plot = q1,
  ...
  ggsave("C:/Users/yzx/Desktop/ElimDA_Paper/ICDE/evaluation/Latency/99.9_latency.pdf", q2, width=10, height=3, device=cairo_pdf)
  # theme_get()$text